...
namespace AssetRipper.Core.Classes.ShaderBlob
{
	public sealed class ShaderSubProgram : IAssetReadable, IAssetWritable
	{
		...
		/// 5.5.0 and greater
		/// </summary>
		public static bool HasUAVParameters(UnityVersion version) => Shader.Shader.IsSerialized(version);
		/// <summary>
		/// 2017.2 and greater
		/// </summary>
		...
		/// 5.5.0 and greater
		/// </summary>
		private static bool HasStatsTempRegister(UnityVersion version) => Shader.Shader.IsSerialized(version);
		/// <summary>
		/// 5.5.0 and greater
		/// </summary>
		private static bool IsAllParamArgs(UnityVersion version) => Shader.Shader.IsSerialized(version);
		/// <summary>
		/// 2017.3 and greater
		/// </summary>
		...
		public void Read(AssetReader reader)
		{
			...
			Shader.ShaderBindChannel[] channels = new Shader.ShaderBindChannel[bindCount];
			for (int i = 0; i < bindCount; i++)
			{
				uint source = reader.ReadUInt32();
				VertexComponent target = (VertexComponent)reader.ReadUInt32();
				Shader.ShaderBindChannel channel = new Shader.ShaderBindChannel(source, target);
				channels[i] = channel;
				sourceMap |= 1 << (int)source;
			}
			BindChannels = new Shader.ParserBindChannels(channels, sourceMap);
...
#warning TODO:
		...
		public Shader.ParserBindChannels BindChannels = new();
	}
}
