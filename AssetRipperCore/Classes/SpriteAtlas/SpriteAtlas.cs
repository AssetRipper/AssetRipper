...
namespace AssetRipper.Classes.SpriteAtlas
{
	public sealed class SpriteAtlas : NamedObject
	{
		...
		public override IEnumerable<PPtr<Object.UnityObject>> FetchDependencies(DependencyContext context)
		{
			...
			{
				foreach (PPtr<Object.UnityObject> asset in context.FetchDependencies(EditorData, EditorDataName))
				{
					yield return asset;
				}
				yield return context.FetchDependency(MasterAtlas, MasterAtlasName);
			}
			foreach (PPtr<Object.UnityObject> asset in context.FetchDependencies(PackedSprites, PackedSpritesName))
			{
				yield return asset;
			}
			foreach (PPtr<Object.UnityObject> asset in context.FetchDependencies((IEnumerable<SpriteAtlasData>)RenderDataMap.Values, RenderDataMapName))
			{
				yield return asset;
			}
		}
