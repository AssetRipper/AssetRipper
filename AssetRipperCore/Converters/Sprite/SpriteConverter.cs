...
namespace AssetRipper.Core.Converters.Sprite
{
	...
	{
		public static SpriteMetaData GenerateSpriteMetaData(IExportContainer container, SpriteAtlas atlas, AssetRipper.Core.Classes.Sprite.Sprite sprite)
		{
			sprite.GetSpriteCoordinatesInAtlas(atlas, out Rectf rect, out Vector2f pivot, out Vector4f border);

			SpriteMetaData instance = new SpriteMetaData(container.ExportVersion);
			instance.Name = sprite.NameString;
			instance.Rect = rect;
			...
			{
				instance.Outline = sprite.GenerateOutline(atlas, rect, pivot);
			}
			if (SpriteMetaData.HasPhysicsShape(container.ExportVersion))
			{
				...
				{
					instance.PhysicsShape = Classes.Sprite.Sprite.HasPhysicsShape(container.Version)
						? sprite.GeneratePhysicsShape(atlas, rect, pivot)
			...
			if (SpriteMetaData.HasBones(container.ExportVersion))
			{
				if (AssetRipper.Core.Classes.Sprite.Sprite.HasBones(container.Version))
				{
					...
					foreach (AssetRipper.Core.Classes.Sprite.SpriteBone bone in sprite.Bones)
					{
						bone.Position *= sprite.PixelsToUnits;
		...
		private static void GetBoneGeometry(IExportContainer container, SpriteMetaData instance, Classes.Sprite.Sprite origin)
		{
