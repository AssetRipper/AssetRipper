                ...
                else
                {
                    ...
                    switch (block.CompressionType)
                    {
                        ...
                        case CompressionType.Lz4HC:
                            ...
                            if (bytesWritten < 0)
                            {
                                DecompressionFailedException.ThrowNoBytesWritten(entry.PathFixed, CompressionType.Lz4HC);
                            }
                            else if (bytesWritten != uncompressedSize)
                            {
                                DecompressionFailedException.ThrowIncorrectNumberBytesWritten(entry.PathFixed, CompressionType.Lz4HC, uncompressedSize, bytesWritten);
                            }
                        ...
                        default:
                            ...
                            {
                                UnsupportedBundleDecompression.Throw(entry.PathFixed, block.CompressionType);
                            }
                            break;
                    }
                    blockStream = m_cachedBlockStream;
