...
public sealed class PredefinedAssetCache
{
	...
	private readonly record struct MeshKey(Utf8String Name, uint VertexCount);
	...
	{
		if (meshDictionary.TryGetValue(new MeshKey(mesh.Name, mesh.VertexData.VertexCount), out AssetMetaPtr assetMetaPtr))
		{
			(fileID, guid, assetType) = assetMetaPtr;
			return true;
		}
	...
	{
		return meshDictionary.TryAdd(new MeshKey(mesh.Name, mesh.VertexCount), new AssetMetaPtr(fileID, guid, assetType));
	}
