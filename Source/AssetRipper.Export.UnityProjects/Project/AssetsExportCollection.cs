...
public abstract class AssetsExportCollection<T> : AssetExportCollection<T> where T : IUnityObjectBase
{
	...
	/// <returns><see langword="true"/> if the <paramref name="asset"/> is added to the <see cref="AssetExportCollection{T}"/> object; <see langword="false"/> if the <paramref name="asset"/> is already present.</returns>
