	...
	public static class RgbConverter
	{
		...
		public unsafe static void RHalfToBGRA32(byte* input, int width, int height, byte* output)
		{
			...
			for (int i = 0; i < width; i++)
			{
				for (int j = 0; j < height; j++)
				{
					double r = Clamp255(Math.Round((float)HalfUtils.ToHalf(sinput[0]) * 255f));
					output[0] = 0;                  // b
					output[1] = 0;                  // g
		...
		public unsafe static void RGHalfToBGRA32(byte* input, int width, int height, byte* output)
		{
			...
			for (int i = 0; i < width; i++)
			{
				...
				{
					double r = Clamp255(Math.Round((float)HalfUtils.ToHalf(sinput[0]) * 255f));
					double g = Clamp255(Math.Round((float)HalfUtils.ToHalf(sinput[1]) * 255f));
					output[0] = 0;                  // b
		...
		public unsafe static void RGBAHalfToBGRA32(byte* input, int width, int height, byte* output)
		{
			...
			for (int i = 0; i < width; i++)
			{
				for (int j = 0; j < height; j++)
				{
					double r = Clamp255(Math.Round((float)HalfUtils.ToHalf(sinput[0]) * 255f));
					double g = Clamp255(Math.Round((float)HalfUtils.ToHalf(sinput[1]) * 255f));
					double b = Clamp255(Math.Round((float)HalfUtils.ToHalf(sinput[2]) * 255f));
					double a = Clamp255(Math.Round((float)HalfUtils.ToHalf(sinput[3]) * 255f));
